    ...
    class Scraper
    {
        public List<string> Scrape(ScrapeCriteria scrapeCriteria)
        {
            ...
            MatchCollection matches = Regex.Matches(scrapeCriteria.Data, 
                scrapeCriteria.Regex, scrapeCriteria.RegexOption);

            foreach(Match m in matches)
            {
                if (!scrapeCriteria.Parts.Any())
                {
                    scrapedElements.Add(m.Groups[0].Value);
                }
                else //element has levels, we go deeper
                {
                    foreach(var part in scrapeCriteria.Parts)
                    {
                        Match matchedPart = Regex.Match(m.Groups[0].Value, part.Regex, part.RegexOption);
